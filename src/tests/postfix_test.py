...
class TestTo_postfix(unittest.TestCase):
    ...
    def test_RPN_division_and_multiplication(self):
        ...
        self.assertEqual(result, deque(['2','5','10','*','4','2','/','-','+']))
    ...
    def test_RPN_power_of(self):
        ...
        self.assertEqual(result, deque(['2', '4', '10', '*', '2', '2', '^', '2', '^', '4', '2', '/', '+', '+', '+']))

    # test for correst order when equation has parentheses
    def test_RPN_parentheses(self):

        result = self.test('(1+2)*(10/2)^(2+1)')

        self.assertEqual(result, deque(['1', '2', '+', '10', '2', '/', '2', '1', '+', '^', '*']))
